pipeline {
   ...
   stage('Git Checkout Frontend') {
            steps {
                dir('frontend') {
                    git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/RaniaWachene1/GreenHouse-Devops-Front.git'
                }
            }
        }
     // Backend Checkout
...
stage('Secret Scanning with GitLeaks') {
    steps {
        dir('backend') {
            sh "docker run -v \$(pwd):/path zricethezav/gitleaks:latest detect --no-git --source='/path' --report-format json --report-path /path/secrets-report.json || true"
        }
    }
}
                ...
                dir('frontend') {
                    script {
                        withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                            sh "docker build -t raniawachene/greenhousefront:latest ."
                        }
                    }
                }
            }
        ...
        stage('Frontend - Push Docker Image') {
            steps {
                dir('frontend') {
                    script {
                        withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                            sh "docker push raniawachene/greenhousefront:latest"
                        }
                    }
                }
            }
